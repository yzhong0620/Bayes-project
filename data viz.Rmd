...
survey_cleaned <- survey %>% 
  ...
  mutate(`Serious Mental Illness` = ifelse(`Serious Mental Illness` == "NO", 0, 1))
```
...
```{r}
logistic <- stan_glm(
  `Serious Mental Illness` ~ Transgender,
  data = survey_cleaned,
  family = binomial,
...
```{r}
prediction_summary(model = logistic, data = survey_cleaned)
```
