...
```{r, include=FALSE, fig.width=10, fig.height=8}
p1 <- survey %>%
  ggplot()+
...
In our to get a sense of our data we first look at the two most basic demographic features we decided to focus on: transgender identity and race. Most people are not transgender. Most of people are WHITE ONLY, following by BLACK ONLY and OTHER. Some proportion of WHITE ONLY and OTHER are Hispanic.
...
p5 <- survey_cleaned %>% 
  ...
  theme(axis.text.x = element_text(angle=20), legend.title = element_blank())

p3 | p4
p5
```

Figure 1: After cleaning we still see that the majority of patients are not transgender. 

Figure 2: This survey collected information on race ("Black only", "white only", "multiracial", or "other") and ethnicity ("Hispanic/Latino" or not) separately. We can see that the sample has about twice as many “white only” respondents as “Black only” respondents. We also see that there are respondents identified as "Hispanic/Latino" in all four racial categories collected, with the highest number in the category “other”.

Figure 3: We can see that a slightly higher percentage of transgender patients were "white only", compared to patients who are not transgender. Just over 50% of both groups were "white only", and around 25% were "Black only". 
...
We can see that there is almost no visible difference between transgender and cisgender (not transgender) patients for any of the four mental health outcomes. We also see that almost our entire sample has mental illness, over 90% have serious mental illness, and around and 8th have alcohol and/or substance disorders. 
...
Figure 8: we see no clear difference in serious mental illness by race

Figure 9: Child patients appear to be less likely than adult patients to have serious mental illness.

Figure 10: The more hours a patient works each week, it appears slightly less likely that they have serious mental illness.

Figure 11: Less education (this does not control for age) appears associated with being less likely to have serious mental illness.

We made many more plots that are not included here, playing with different arrangements of these variables. The one that was interesting enough to include looked at both transgender identity and age as predictors of serious mental illness.
...
Figure 12: Here we see that while there is no difference in serious mental illness between trans and cis patients among adults, child patients who are transgender appear considerably more likely than their cis peers to have serious mental illness. 
...
\\
&\text{where }\text{log}(\frac{\pi_i}{1-\pi_i})=\beta_0 + \beta_1\text{RaceMULTI} + \beta_2\text{RaceOTHER} + \beta_3\text{RaceWHITE}
\\
&\beta_0\sim\text{N}(0, s_0^2)
\\
&\beta_1\sim\text{N}(0, s_1^2)
...
We chose a weakly informative prior with coefficients centered around 0 with wide variances. The predictor Race has 4 categories.
...
\\
&\text{where }\text{log}(\frac{\pi_i}{1-\pi_i})=\beta_0 + \beta_1\text{Transgender} + \beta_2\text{RaceMULTI} + \beta_3\text{RaceOTHER} + \beta_4\text{RaceWHITE}
\\
&+ \beta_5\text{SexualOrientationLESBIAN|GAY} +\beta_6\text{SexualOrientationOTHER} + \beta_7\text{SexualOrientationSTRAIGHT|HETEROSEXUAL}
\\
&\beta_0\sim\text{N}(0, s_0^2)
\\
&\beta_1\sim\text{N}(0, s_1^2)
...
Overall we found that being a child, identifying race as "white only" or "other", having less than a college degree, and/or working more hours per week were associated with less chance of having serious mental illness. While age and hours worked and education level (as discussed above) may be inherently related to serious mental illness and/or this population, the relevance of race may point to systemic man-made factors that could be changed to promote better mental health for everyone.
